function [ITE,MTE]=dblmnonrigidmaskgen_1(PlanCTR,PreCBCTData,PreCBCTLoca,PlanMask1)
...
% lambda3=0.00001;
lambda=1;
lambda1A=0.05;% Distance Regularisation
lambda3=0.001;
...
%     mV2=BL3DSecndPass_stripped_2_inbuilt(imgTreatment,imgPlanning,mV1,mbSize,p,ZLocV,indz,indL,lambda1A,lambda3,m,n,LenX,LenY,costV); % 2nd Pass
    ...
    mV1=BL3DFirstPass_stripped_3_inbuilt(IT1(:,:,ind),IP11(:,:,indz),mbSize,p,ZLocV,indz,indL,lambda,lambda1A,m,n,costV); % 1st Pass
    mV2=BL3DSecndPass_stripped_2_inbuilt(IT1(:,:,ind),IP11(:,:,indz),mV1,mbSize,p,ZLocV,indz,indL,lambda,lambda1A,lambda3,m,n,LenX,LenY,costV); % 2nd Pass    
    
%     MTE1=motioncomp3d_1_inbuilt(masPlanning,mV1,m,n,indL,mbSize);% Mask prediction - 1st Pass
    MTE2=motioncomp3d_1_inbuilt(MP11(:,:,indz),mV2,m,n,indL,mbSize);% Mask Prediction - 2nd Pass
    
%     MTE1=dustthemask(MTE1);% Mask Correction
    MTE2=dustthemask(MTE2,m,n);
%     if ~any(MTE2(:))==0
%         MTE2=activecontour(IT1(:,:,ind),MTE2,300);
%         MTE(:,:,ind)=MTE2;
%     else
%     MTE(:,:,ind)=MTE2;
%     end
%     ITEsl=motioncomp3d_1_inbuilt(imgPlanning,mV2,m,n,indL,mbSize);
    ITEsl=motioncomp3d_1_inbuilt(IP11(:,:,indz),mV2,m,n,indL,mbSize);
...
%                 ITE(:,:,ind)=imresize(ITEsl,[resm resn]);
                    ITE(:,:,ind)=ITEsl;
                    MTE(:,:,ind)=MTE2;
    end % End loop for ind
MTE=imresize3(MTE, [resm resn ZL]);
ITE=imresize3(ITE, [resm resn ZL]);
% for ind=1:ZL
%     MTE2=MTE(:,:,ind);
%     if ~any(MTE2(:))==0
%         MTE2=activecontour(PreCBCTData(:,:,ind),MTE2,300);
%         MTE(:,:,ind)=MTE2;
%     else
%     MTE(:,:,ind)=MTE2;
%     end
...
function mV1=BL3DFirstPass_stripped_3_inbuilt(imgTreatment,imgPlanning,mbSize,p,ZLocV,indz,indL,lambda,lambda1A,m,n,costV)
...
% imgTreatment=it;
% imgPlanning=mp;
impSiz=size(imgPlanning);
SSM=zeros(impSiz(1),impSiz(2),impSiz(3));
for si=1:impSiz(3)
    [~,SSM(:,:,si)] = ssim(imgTreatment,imgPlanning(:,:,si));
    SSM(:,:,si)=(1-SSM(:,:,si))*lambda;
end
%         SSM(:,:,si)=ssimmap;
...
%                     refBlk1=imgPlanning(refBlkVer:refBlkVer+mbSize-1,refBlkHor:(refBlkHor+mbSize-1),zi);
                    refBlk1=SSM(refBlkVer:refBlkVer+mbSize-1,refBlkHor:(refBlkHor+mbSize-1),zi);
                    err1=sum(refBlk1(:));
%                 [err1,~]=ssim(currentBlk,refBlk1);
%                 err1=sum(abs(currentBlk(:)-refBlk1(:)));
%                 err1=vectorMI(currentBlk,refBlk1,mbSize_N);
%                 [Hp,~]=imhist(currentBlk);
%                 [Ht,~]=imhist(refBlk1);
%                 Hp=Hp(Hp~=0);
%                 Ht=Ht(Ht~=0);
%                 Hp=Hp/sum(Hp);
...
%                 Sp=-sum(Hp.*log2(Hp));
%                 St=-sum(Ht.*log2(Ht));
% %                 St=entropy(currentBlk);
% %                 Sp=entropy(refBlk1);
%                 [~,~,uscBlk]=unique(currentBlk(:));
%                 [~,~,usrBlk]=unique(refBlk1(:));
%                 jointprob = accumarray([uscBlk usrBlk], 1); 
%                 jointprob = jointprob / mbSize_N; 
...
function mV2=BL3DSecndPass_stripped_2_inbuilt(imgTreatment,imgPlanning,mV1,mbSize,p,ZLocV,indz,indL,lambda,lambda1A,lambda3,m,n,LenX,LenY,costV)
% it=imresize(imgTreatment,[m n]);
% mp=zeros(m,n,indL);
%     for si=1:indL
%     mp(:,:,si)=imresize(imgPlanning(:,:,si),[m n]);
