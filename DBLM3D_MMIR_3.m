...
tic;
[geomtform,Rfixed,Rmoving,optimizer,metric]=dblmrigidimregister(PreCBCTData,PlanCTData,PreCBCTInfo{1,1},PlanCTInfo{1,1});
timeR=toc;
%% Warping image and masks after rigid
PlanCTR = imwarp(PlanCTData,Rmoving,geomtform,'bicubic','OutputView',Rfixed);

% NoC=size(ContourName);
% PlanCTMaskcellR=cell(NoC(1),1);
% for contouri=1:NoC(1)
%     PlanCTMaskcellR{contouri,1}=imwarp(PlanCTMaskcell{contouri,1},Rmoving,geomtform,'bicubic','OutputView',Rfixed);
% end
%% DBLM - stage (Stage 2 Non-rigid registration)
m=500;
n=500;
...
function [geomtform,Rfixed,Rmoving,optimizer,metric]=dblmrigidimregister(fixedVolume,movingVolume,fixedHeader,movingHeader)
