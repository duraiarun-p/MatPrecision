...
IP1=int16(imresize3(PlanCTR, [m n ZL]));
IT1=int16(imresize3(PreCBCTData, [m n ZL]));
PreCBCTLoca=int16(PreCBCTLoca);
%%
IPg=gpuArray(IP1);
ITg=gpuArray(IT1);
Z1=gpuArray(PreCBCTLoca);
%%


ITE=int16(zeros(m,n,ZL));
tic;
ind=1;
%     for ind=1:ZL
...
%         imgTreatment=ITg(:,:,ind);
%     imgPlanning=IPg(:,:,indz);
    % masTreatment=MT2(:,:,ind);
%     masPlanning=MP2(:,:,indz);

    mV1=BL3DFirstPass_stripped_3_inbuilt(ITg(:,:,ind),IPg(:,:,indz),mbSize,p,ZLocV,indz,indL,lambda1A,m,n,costV); % 1st Pass
%     mV2=BL3DSecndPass_stripped_2_inbuilt(ITg(:,:,ind),IPg(:,:,indz),mV1,mbSize,p,ZLocV,indz,indL,lambda1A,lambda3,m,n,LenX,LenY,costV); % 2nd Pass
...
costs1 = gpuArray(int16(ones(prow, prow, indL) * costV));%Cost matrix
dist = gpuArray(int16(zeros(prow, prow, indL)));

mbCount = 1;
mbSize_N=mbSize*mbSize;
% mV1 = zeros(round(m*n/mbSize^2),7); % 3D motion vector + Z index
mV1 = gpuArray(zeros(round(m*n/mbSize^2),7));
blkfactor=1;
% for zi=1:indL
